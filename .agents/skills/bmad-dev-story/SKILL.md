...
Run: ``${BMAD_STORY_AUTOMATOR_PYTHON:-python3} {project-root}/_bmad/scripts/resolve_customization.py --skill {skill-root} --key workflow``
...
<workflow>
  ...
  <action>Run: ``${BMAD_STORY_AUTOMATOR_PYTHON:-python3} {project-root}/_bmad/scripts/resolve_customization.py --skill {skill-root} --key workflow.on_complete`` — if the resolved value is non-empty, follow it as the final terminal instruction before exiting.</action>
  </step>
