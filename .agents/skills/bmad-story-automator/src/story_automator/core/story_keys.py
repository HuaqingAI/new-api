...
def normalize_story_key(project_root: str, value: str, state_file: str | Path | None = None) -> StoryKey | None:
    ...
    if not key:
        ...
        if file_exists(status_file):
            ...
            status_key = _status_key_for_prefix(content, prefix, story_id)
            if status_key:
                key = status_key
    if not key:
        key = prefix
...
def normalize_story_key_for_epic(project_root: str, epic: str, value: str) -> StoryKey | None:
    ...
    if dotted:
        ...
        return _complete_story_key_for_epic(project_root, epic, story_num)
    ...
    if dashed:
        ...
        norm = _complete_story_key_for_epic(project_root, epic, story_num)
        if norm is None:
            return None
...
def _complete_story_key_for_epic(project_root: str, epic: str, story_num: str) -> StoryKey | None:
...
def _story_keys_in_epic_block(content: str, context_slug: str, epic_num: str) -> list[str]:
    ...
    for raw_line in lines:
        ...
        if not inside_block:
            inside_block = key.casefold() == epic_header.casefold()
            continue
...
def _status_key_for_prefix(content: str, prefix: str, story_id: str) -> str:
    for status_key in _status_keys(content):
        if status_key.casefold().startswith(f"{prefix.casefold()}-") and _full_key_matches_story("", status_key, story_id, allow_ambiguous_same_id=True):
            return status_key
    return ""
