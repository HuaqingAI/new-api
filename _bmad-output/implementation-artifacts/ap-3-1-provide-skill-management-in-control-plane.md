...
- [x] 让 Skill 治理动作与现有 lifecycle / exposure 模型连起来 (AC: 2, 3)
  ...
  - [x] 明确 Skill 未发布时不会被 open capability discovery 发现，行为继续以 exposure 为 source of truth，本故事不改变这条边界。[Source: _bmad-output/planning-artifacts/epics-agent-platform.md#Story 3.1; _bmad-output/implementation-artifacts/ap-1-5-build-publication-projection-and-exposure-model.md]
  - [x] 未在 3.1 提前实现 Skill invoke 业务执行，invoke 契约和执行闭环继续留给 3.2/3.3。[Source: _bmad-output/planning-artifacts/epics-agent-platform.md#Story 3.2; #Story 3.3]

- [x] 在 web/default 的 Agent Platform shell 中接入 Skill 管理占位到真实数据视图 (AC: 1, 2)
  - [x] 将 `web/default/src/features/agent-platform` 中当前 “Pending Epic 3” 的 Skills 区域替换为最小 Skill 管理列表承接面，能展示已存在 Skill 的名称、状态、当前版本、owner 信息。[Source: web/default/src/features/agent-platform/index.tsx]
...
- [Source: _bmad-output/planning-artifacts/epics-agent-platform.md#Story 3.1: 在控制面提供 Skill 管理能力]
...
- Story Automator 继续作为总编排状态源，但 `3.1` 采用 manual create/dev takeover，避免 create-story / dev-story 子进程在 Agent Platform Epic 3 上继续漂移。
- 已加载 Epic 3 需求、FR-7/FR-8、现有 `resource/resource_version/skill_def` 实现和 web/default Agent Platform shell，准备补 Skill 控制面专属入口。
