...
团队在 Epic 7 完成后，对企业治理体验做纠偏：补齐治理工作台 i18n / action label、把员工额度申请入口迁移到普通用户余额/钱包场景，并把预算池类型约束从“部门级不可变”收敛回“单池不可变”。

**FRs covered:** follow-up to FR9, FR25, FR26
...
**Given** 用户在非英文语言下查看治理时间线、通知投递和员工额度申请区域
**When** 页面渲染标题、表头和空状态文案
**Then** 文案全部走 i18n key
**And** 不出现漏翻译英文。

**Given** 时间线中存在治理动作记录
**When** 页面展示 action 列
**Then** `action_type` 会被映射成可读标签
**And** 不直接回退为内部 action key，除非明确进入兜底状态。
...
**And** 提交后继续复用现有 quota request workflow。

**Given** 负责人或管理员进入企业组织工作台
**When** 查看当前部门治理区域
**Then** 仍能处理审批、查看治理时间线和通知状态
**And** 不丢失现有治理闭环能力。
