...
func TestGenerateOAuthCodeSupportsDingTalkAuthFlow(t *testing.T) {
	setupAuthFlowControllerTest(t)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/api/oauth/state", strings.NewReader(`{"provider":"dingtalk","intent":"login"}`))
	c.Request.Header.Set("Content-Type", "application/json")
	...
	require.True(t, response.Success)
	flow, err := model.GetAuthFlow(response.Data.FlowToken, model.AuthFlowMatch{
		Purpose: model.AuthFlowPurposeOAuth, Provider: "dingtalk", Intent: model.AuthFlowIntentLogin,
	})
	require.NoError(t, err)
	assert.Equal(t, "dingtalk", flow.Provider)
}
