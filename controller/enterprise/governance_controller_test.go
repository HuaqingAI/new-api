...
func TestGovernanceNotificationUnconfiguredListAndResendBoundary(t *testing.T) {
	router, db := setupEnterpriseControllerTest(t)
	...
	list := performEnterpriseRequest(t, router, http.MethodGet, "/api/enterprise/governance/notifications?department_id=1&status=unconfigured", nil)
	listResponse := decodeEnterpriseAPIResponse(t, list)
	require.True(t, listResponse.Success, listResponse.Message)
	listPayload := decodeEnterpriseData[dtoenterprise.GovernanceNotificationResponse](t, listResponse)
	require.Len(t, listPayload.Items, 1)
	require.Equal(t, entmodel.GovernanceNotificationStatusUnconfigured, listPayload.Items[0].Status)
	require.Equal(t, "fulfilled", listPayload.Items[0].Trace.Status)
	require.Equal(t, int64(0), listPayload.Items[0].NextRetryAt)

	resend := performEnterpriseRequest(t, router, http.MethodPost, "/api/enterprise/governance/notifications/1/resend", nil)
	resendResponse := decodeEnterpriseAPIResponse(t, resend)
	require.False(t, resendResponse.Success)
	require.Equal(t, "common.forbidden", resendResponse.Message)

	var manualCount int64
	require.NoError(t, db.Model(&entmodel.GovernanceNotificationDelivery{}).
		Where("manual_parent_id = ? AND trigger_source = ?", delivery.Id, entmodel.GovernanceNotificationTriggerManual).
		Count(&manualCount).Error)
	require.Equal(t, int64(0), manualCount)
}
