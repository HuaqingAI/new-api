...
| Skill invoke | signed off | `docs/openapi/api.json`; `tests/agentplatform/conformance/fixtures.go`; `controller/agentplatform/open_capabilities_test.go`; `service/agentplatform/skill_invoke_test.go` | AP-6.4 已冻结 sync-only invoke request/response、`output` shape、timeout / upstream failure / contract invalid error convergence，并显式排除 async / cancel / task-status 出 P0。 |
