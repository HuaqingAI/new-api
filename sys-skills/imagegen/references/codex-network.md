...
This file is for the local CLI path. The skill runs `scripts/image_gen.py`
directly against the configured new-api OpenAI-compatible Images API.

This guidance is intentionally isolated from `SKILL.md` because it can vary by environment and may become stale. Prefer the defaults in your environment when in doubt.

## Why am I asked to approve image generation calls?
The CLI calls a network API endpoint such as `{base_url}/images/generations`
or `{base_url}/images/edits`, so it needs outbound network access. In many
Codex setups, network access is disabled by default and/or the approval policy
