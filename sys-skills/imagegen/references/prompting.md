...
These prompting principles apply to this skill's local CLI path:
- `scripts/image_gen.py` calling the configured new-api OpenAI-compatible Images API

This file is about prompt structure, specificity, and iteration. CLI execution
controls such as `quality`, masks, output format, output paths, and continuation
state live in `references/cli.md` and `references/image-api.md`.
...
## Transparent images
- This skill is fixed to `gpt-image-2` and does not support `background=transparent`.
- Do not switch to another model for native transparency.
- If the user needs alpha, explain the limitation and use a solid background or local post-processing only when that tradeoff is acceptable.

## CLI execution controls
- `quality`, explicit masks, output format, output paths, and `--from-last` are CLI execution controls.
- See `references/cli.md` and `references/image-api.md` for the supported controls.
- The model is fixed to `gpt-image-2`. It supports `quality=low|medium|high|auto`; use `low` for fast drafts and thumbnails, and move to `medium`, `high`, or `auto` for final assets.
- `gpt-image-2` always uses high fidelity for image inputs, so do not set `input_fidelity`.
- This fixed-model skill does not support `background=transparent`.
...
- background-extraction: This fixed-model skill cannot request native transparency; ask for a clean solid background or use local post-processing only when acceptable; preserve fine edges and label text; no halos or restyling.
