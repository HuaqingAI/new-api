...
These prompt recipes are for this skill's local CLI path:
...
Execution details such as CLI flags, `quality`, masks, output formats, local
output paths, and `--from-last` live in `references/cli.md`. Do not use the
built-in `image_gen` tool or AionUi's built-in image MCP for this skill.

CLI model notes:
- `gpt-image-2` is fixed for all generation and edit requests.
...
- Do not set `input_fidelity`; image inputs already use high fidelity.
