...
describe('API key group table cell', () => {
  ...
  test('renders an unclipped ratio ring and localized Auto ratio when API data uses a nonlocalized string', async () => {
  ...
  test('shows the Cross-group badge when ratio data is unavailable', async () => {
    const container = document.createElement('div')
    document.body.append(container)
    const root = createRoot(container)
